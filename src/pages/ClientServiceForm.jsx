...
export default function ClientServiceForm() {
    ...
    const getPetowners = () => {
        ...
        // setLoading(true);
        axiosClient.get('/clientservices')
            .then(({ data }) => {
                setLoading(false);
...
getPetowners();
    ...
    const onSubmit = (ev) => {
        ev.preventDefault();
        if (clientservice.id) {
            axiosClient.put(`/clientservices/${clientservice.id}`, clientservice)
                .then(() => {
            ...
            axiosClient.post(`/clientservices`, clientservice)
                .then(() => {
    ...
    return (
        <div>
            ...
            {!clientservice.id && <h1 className="title">Client Service (Consent For Treatment)</h1>}
                ...
                {!loading && (
                    <form onSubmit={onSubmit}>
                      ...
                      <select
                            ...
                            onChange={(ev) =>
                                setClientService({ ...clientservice, petowner_id: ev.target.value })
                            }
                            >
                        ...
                        <Link className="btn" to="/clientservices">Back</Link>
                    </form>
                )}
            </div>
        </div>
