...
export default function AppointmentArchives() {
  const columns = [
    { id: "id", name: "ID" },
  ...
  const getAppointmentArchives = () => {
    setMessage(null);
    setAppointments([]);
    setLoading(true);
    axiosClient
      .get("/archives/appointments")
      .then(({ data }) => {
        setLoading(false);
  ...
  const onRestore = (po) => {
    ...
    axiosClient.put(`/appointments/${po.id}/restore`).then(() => {
      setNotification("Vet was successfully restored");
      getAppointmentArchives();
    });
  };
